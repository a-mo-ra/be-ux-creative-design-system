  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h2 className="text-2xl font-bold text-neutral-900">Biblioteca de Ícones</h2>
          <p className="text-neutral-600">Coleção completa com Lucide React e ícones customizados</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {category.icons.map((icon) => {
              ...
              return (
                <div
                  key={icon.name}
                  className="group p-4 border border-neutral-200 rounded-lg hover:shadow-elevation-2 hover:border-brand-200 transition-all duration-200 cursor-pointer"
                  onClick={() => copyIconCode(icon.name)}
                >
                  <div className="flex flex-col items-center space-y-3">
                    <div className="w-12 h-12 bg-neutral-50 rounded-lg flex items-center justify-center group-hover:bg-brand-50 transition-colors duration-200">
                      <IconComponent 
                        size={24} 
                        className="text-neutral-600 group-hover:text-brand-600 transition-colors duration-200" 
                      />
                    </div>
                    
                    <div className="text-center">
                      <p className="text-xs font-medium text-neutral-800 mb-1">{icon.name}</p>
                      <p className="text-xs text-neutral-500">{icon.usage}</p>
                    </div>
      ...
      {/* Icon Sizes Demo */}
      <div className="bg-white rounded-xl p-6 shadow-elevation-2 border border-neutral-200/50">
        <h3 className="text-lg font-semibold text-neutral-900 mb-6">Tamanhos de Ícones</h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            ...
            <div key={item.size} className="text-center p-4 border border-neutral-200 rounded-lg">
              <div className="w-16 h-16 bg-neutral-50 rounded-lg flex items-center justify-center mx-auto mb-3">
                <Heart size={item.size} className="text-brand-500" />
              </div>
              <h4 className="font-medium text-neutral-800 mb-1">{item.name}</h4>
              <p className="text-xs text-neutral-600 mb-2">{item.size}px</p>
              <p className="text-xs text-neutral-500">{item.usage}</p>
              <code className="text-xs bg-neutral-100 px-2 py-1 rounded mt-2 block">
                size={'{' + item.size + '}'}
              </code>
            </div>
          ))}
      ...
      <div className="bg-gradient-to-br from-brand-50 to-white rounded-xl p-6 border border-brand-200/50">
        ...
        <div className="grid md:grid-cols-2 gap-6 text-sm">
          <div>
            <h4 className="font-medium text-neutral-800 mb-3">Consistência</h4>
